...
fn build_app_menu(handle: &tauri::AppHandle, recent_projects: &[RecentProject]) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let edit_menu = Submenu::with_items(
        ...
        &[
            ...
            &PredefinedMenuItem::separator(handle)?,
            &MenuItem::with_id(handle, "menu_cut_node", "Cut", false, None::<&str>)?,
            &MenuItem::with_id(handle, "menu_copy_node", "Copy", false, None::<&str>)?,
            &MenuItem::with_id(handle, "menu_paste_node", "Paste", false, None::<&str>)?,
